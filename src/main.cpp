...
int main() {
    ...
    while (cap.isOpened() && ws_is_ready())
    {
        ...
        if (laneInfo.status == "Detected") { statusText = "TERDETEKSI"; statusColor = cv::Scalar(0, 250, 0); }
        else if (laneInfo.status == "Partial") { statusText = "SEBAGIAN"; statusColor = cv::Scalar(0, 250, 250); }
        else { statusText = "HILANG"; statusColor = cv::Scalar(0, 0, 250); }
        ...
        cv::rectangle(output, cv::Rect(cv::Point(20, 170), cv::Point(20 + gaugeWidth, 190)), cv::Scalar(0, 250, 0), -1);
        
        int y_pos = 40;
        cv::putText(output, fpsText, cv::Point(20, y_pos), cv::FONT_HERSHEY_SIMPLEX, 0.7, cv::Scalar(0, 250, 0), 2);
        y_pos += 30;
        cv::putText(output, "Status: " + statusText, cv::Point(20, y_pos), cv::FONT_HERSHEY_SIMPLEX, 0.7, statusColor, 2);
        y_pos += 30;
        cv::putText(output, angleText, cv::Point(20, y_pos), cv::FONT_HERSHEY_SIMPLEX, 0.7, cv::Scalar(250, 250, 0), 2);
        y_pos += 30;
        cv::putText(output, distText, cv::Point(20, y_pos), cv::FONT_HERSHEY_SIMPLEX, 0.7, cv::Scalar(250, 250, 0), 2);
        y_pos += 60;
        cv::putText(output, speedText, cv::Point(20, y_pos), cv::FONT_HERSHEY_SIMPLEX, 0.7, cv::Scalar(250, 250, 250), 2);
